from sqlalchemy import Enum, String, Boolean, Integer, Text, JSON
...
class Command(Base):
  ...
  has_context: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
  ...
  @staticmethod
  def _validate_fallback(val: str = None):
  ...
  @property
  def params(self):
    return dict(method=self.method.value, endpoint=self.endpoint, cooldown=self.cooldown, name=self.name, enabled=self.enabled, help=self.help, aliases=self.alias)
